...
**mcp-alm** — TypeScript / Node (ESM), pięć serwerów MCP per integracja ALM: `mcp-jira`, `mcp-confluence`, `mcp-figma`, `mcp-sonar`, `mcp-gitlab`. Cloud + Data Center / SaaS + self-hosted.
...
> Pełny opis: [`docs/tech-stack.md`](../docs/tech-stack.md). Blok generowany przez `npm run stack:sync` — nie edytuj ręcznie.
...
> Konflikt między regułami tego samego poziomu rozwiązuj porządkiem: `core` > `principles` > domenowe (`security`, `tokens`, `mcp-server`, `connectors`) > `language`. User prompt wygrywa zawsze.
