...
- **Nie zapisuje findings do Copilot Memory.** Copilot Memory (od 2026-05-26) cache'uje notes per-scope. Security findings często zawierają konkretne tokens / API keys / project keys — to PII-class data, której nie chcemy w persistent memory store. Jeśli używasz `/memory show` w Copilot CLI, zweryfikuj że żadne finding z tego audytu tam nie wpadło. Findings logujemy tylko w git-tracked plikach z konkretnym retention plan.
