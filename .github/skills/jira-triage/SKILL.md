---
name: jira-triage
description: Triage a set of Jira issues — group by priority/severity, component, and epic, then propose an order of attack — using the read-only mcp-jira tools (jira.search_issues, jira.bulk_get_issues, jira.get_issue). Use when the user asks to triage, group, or prioritise a backlog, sprint, or JQL filter (e.g. "triage these bugs", "group open issues by component"). Read-only.
---
...
- `keys` — lista konkretnych kluczy (np. `ABC-1, ABC-2`).
