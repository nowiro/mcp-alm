...
- `mcp-jira`, `mcp-confluence`, `mcp-figma`, `mcp-sonar`, `mcp-gitlab` — ~67 narzędzi (read + write-guarded), każde z Zod I/O. `mcp-jira` pokrywa też Agile: `list_boards` / `list_sprints` / `get_sprint` / `get_sprint_issues`.
