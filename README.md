...
3. Opisz zadanie — np. _"Dodaj `jira.list_components` z parametrem `projectKey`, zwraca komponenty projektu + leadów"_ albo _"Rozbij epic PROJ-100 na stories per INVEST z cut na 3 sprinty"_.
...
npm run workflow:add-tool -- \
  ...
  --description="List a project's components + their leads." \
  --type=read
# → docs/runs/<date>-add-tool-jira-list_components.md
