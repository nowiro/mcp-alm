...
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `mcp-jira`       | `get_issue`, `search_issues`, `bulk_get_issues`, `approximate_count`, `list_projects`, `list_boards`, `list_sprints`, `get_sprint`, `list_transitions`, `get_comments`, `get_worklogs`, `get_changelog`, `jql_builder`, `health` | `create_issue`, `bulk_create_issues`, `transition_issue`, `add_comment`, `add_worklog`, `link_issues` |
...
3. Opisz zadanie — np. _"Dodaj `jira.get_sprint_issues` z parametrami `sprintId`, `fields`, zwraca canonical issue shapes"_ albo _"Rozbij epic PROJ-100 na stories per INVEST z cut na 3 sprinty"_.
...
npm run workflow:add-tool -- \
  ...
  --description="List issues in a sprint (canonical issue shape)." \
  --type=read
# → docs/runs/<date>-add-tool-jira-get_sprint_issues.md
